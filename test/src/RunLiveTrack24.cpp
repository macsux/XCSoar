...
#include "Tracking/LiveTrack24API.hpp"
...
TestTracking(int argc, char *argv[])
{
  ...
  while (replay->Next()) {
    ...
    result = SendPosition(
        session, package_id, basic.location, basic.NavAltitude,
        Units::ToUserUnit(basic.ground_speed, unKiloMeterPerHour), basic.track,
        datetime.ToUnixTimeUTC());
